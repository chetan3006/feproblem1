...
export default function Dashboard({result,vehicledata,planetsdata,addDataToForm,updateData,time,calculateAndUpdateTime,formdata,findFalcone}){
    ...
    const sendData=()=>{
        navigate('/result',{state:{time:time}});
    }
    ...
    return(
        <div><Stack>
            ...
            <div>
            <Button onClick={()=>findFalconeClick()} disabled={(formdata.planet_names.length===4)?false:true} variant="contained" fullWidth={false}>Find Falcone</Button>
            </div>
            </Stack>
